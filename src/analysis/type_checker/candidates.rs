...
impl Candidates {
    pub fn meet(&self, others: &Candidates) -> Candidates {
        match (self, others) {
            ...
            (Candidates::Concrete(c), Candidates::Numeric(cfg))
            | (Candidates::Numeric(cfg), Candidates::Concrete(c)) => {
                Candidates::meet_conc_abs(*c, *cfg)
            }
            (Candidates::Concrete(t0), Candidates::Concrete(t1)) => {
                Candidates::meet_builtin(*t0, *t1)
            }
    ...
    fn meet_builtin(t0: BuiltinType, t1: BuiltinType) -> Candidates {
        match (t0, t1) {
            (BuiltinType::Float(w0), BuiltinType::Float(w1)) => {
                Candidates::Concrete(BuiltinType::Float(std::cmp::max(w0, w1)))
            }
            (BuiltinType::Int(w0), BuiltinType::Int(w1)) => {
                Candidates::Concrete(BuiltinType::Int(std::cmp::max(w0, w1)))
            }
            (BuiltinType::UInt(w0), BuiltinType::UInt(w1)) => {
                Candidates::Concrete(BuiltinType::UInt(std::cmp::max(w0, w1)))
            }
    ...
    pub fn is_numeric(&self) -> bool {
        match self {
            Candidates::Numeric(_)
            | Candidates::Concrete(BuiltinType::Float(_))
            | Candidates::Concrete(BuiltinType::UInt(_))
            | Candidates::Concrete(BuiltinType::Int(_)) => true,
    ...
    pub fn into_signed(self) -> Candidates {
        match self {
            Candidates::Numeric(cfg) => Candidates::Numeric(NumConfig {
                width: cfg.width,
                def_signed: true,
    ...
    pub fn width(&self) -> Option<u8> {
        match self {
            ...
            Candidates::Concrete(BuiltinType::UInt(w))
            | Candidates::Concrete(BuiltinType::Int(w))
