...
mod tests {
    ...
    fn simple_input() {
        ...
        assert!(!handler.contains_error(), "There should not be a typing error.");
    }
    ...
    fn simple_const_faulty() {
        ...
        let (_, handler) = setup(spec);
        assert_eq!(handler.emitted_errors(), 1, "Incompatible types were not recognized as such.");
    }
    ...
    fn test_window_untimed() {
        ...
        let (_, handler) = setup(spec);
        assert_eq!(handler.emitted_errors(), 1, "Incompatible timings were not recognized as such.");
    }
